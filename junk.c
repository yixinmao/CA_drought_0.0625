...
int main (int argc, char *argv[])   // average T (Nov 1 - Mar 19) for each year [deg C]
{
  ...
  for(i=0;i<nfiles;i++){
	...
	for(j=0;j<ndays;j++){
      ...
      else if(month<=2 || (month==3 && day<=23))  // if Jan - Mar 23
      {
	...
	for(j=0;j<ndays;j++){
	  ...
	  else if((month<=2 || (month==3 && day<=23)) && prec>=rain_thred_year[year-syear])  // if Jan - Mar 23 && rainy, add it to this water year
	  {
...
//		if(j+syear==2013) printf("%d %f %f\n", j+syear, T_anom[j], T_anom_ave_grid[j]);
	}
    printf("%d %f\n", j, T_anom_ave_grid[0]);
    printf("%d %f\n", j, T_anom_ave_grid[j-1]);
  }
    fclose(fplist);
