...
#define NLEAP 24  // number of leap years in the record

double get_dist(double, double, double, double);
double calc_area(float lat, float lon, float cellsize);
int get_length(FILE *file);
double largest_rain_threshold(int n, double *data, int nlargest);

int main (int argc, char *argv[])   // for each grid and each year, only average Tanom over the 5 most-rainy days
{
  ...
  double prec_data[MAXY][500];
  double rain_thred_year[MAXY]; // the threshold for 5 most-rainy day in each year for each file [mm]
  ...
  for(i=0;i<nfiles;i++){
    ...
    /////// calculate ave T on each day (Nov1 - Apr1)
    for(j=0; j<MAXMON; j++)
		for(k=0; k<MAXDAY; k++)
		{
			ave_T[j][k] = 0;
    ...
    for(j=0; j<MAXMON; j++)
		for(k=0; k<MAXDAY; k++)
		{
			...
			else if((j+1<=4) || ((j+1==4) && (k+1<=1))) ave_T[j][k] = ave_T[j][k] / nyear;
		}

    ///////// find the threshold for 5 days in each year with the highest precipitation
	for(j=0; j<=MAXY; j++)
	{
		nday_year[j] = 0;
		for(k=0; k<500; k++)
	...
	for(j=0;j<ndays;j++){
      ...
      fscanf(fpin,"%lf %lf %lf %lf",&prec,&evap,&runoff,&baseflow);
      fscanf(fpin,"%lf %lf %lf %lf %lf",&airT,&sm1,&sm2,&sm3,&swe);
      if(month>=11)   // if Nov or Dec
      {
         prec_data[year-syear+1][nday_year[year-syear+1]] = prec;
         nday_year[year-syear+1]++;
      }
      else if(month<=3 || (month==4 && day<=1))  // if Jan - Apr 1
      {
         prec_data[year-syear][nday_year[year-syear]] = prec;
    ...
    for(j=0; j<nyear; j++)
    {
       rain_thred_year[j] = largest_rain_threshold(nday_year[j], prec_data[j], 5);
    }
	...
	for(j=0;j<ndays;j++){
      ...
      if(month>=11 && prec>=rain_thred_year[year-syear+1])   // if Nov or Dec && rainy, add it to the next water year
	  {
		T_anom[year-syear+1] += anom;
		nday_year[year-syear+1]++;
	  }
	  else if((month<=3 || (month==4 && day<=1)) && prec>=rain_thred_year[year-syear])  // if Jan - Apr 1 && rainy, add it to this water year
	  {
	...
	for(j=0; j<nyear; j++) 
	{
		...
		if(j+syear==2013)  printf("%d %f %f %f\n", j+syear, T_anom[j], area, T_anom_ave_grid[j]);
	}
...
double largest_rain_threshold(int n, double *data, int nlargest)  // length of data; data; find the largest 'nlargest data'; retrun: a threshold for the largest nlargest data
{
	int i, j;
	double *data_sorted;
	double temp;
	...
	data_sorted = data;
	for(i=n-1; i>=n-nlargest; i--)
	{
		for(j=0; j<=i-1; j++)
		{
			if(data_sorted[j]>data_sorted[j+1])  // switch
			{
				temp = data_sorted[j];
