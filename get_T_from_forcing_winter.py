...
for i in range(nfiles):
	...
	for t in range(nday):
		...
		if month>=11:  # if Nov-Dec, add it to the next water year
			Tmax_year[year_ind+1] = Tmax_year[year_ind+1] + area*Tmax
			Tmin_year[year_ind+1] = Tmin_year[year_ind+1] + area*Tmin
			nday_year[year_ind+1] = nday_year[year_ind+1] + 1
		elif month<=3:  # if Jan-Mar, add it to this water year
