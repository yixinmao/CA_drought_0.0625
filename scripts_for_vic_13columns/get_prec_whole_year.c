...
int main (int argc, char *argv[])  // total prec for each year [km3]
{
  ...
  for(i=0;i<nfiles;i++){
    ...
    for(j=0;j<ndays;j++){
      ...
      if(j==0)syear=year;
      fscanf(fpin,"%lf %lf %lf %lf %lf",&prec,&evap,&runoff,&baseflow);
      fscanf(fpin,"%lf %lf %lf %lf %lf",&airT,&sm1,&sm2,&sm3,&swe,&sw);
