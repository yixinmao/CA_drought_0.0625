...
int main (int argc, char *argv[])   // accumulative prec (Apr - Oct) for each year [km3]
{
  ...
  double airT,sm1,sm2,sm3,swe,sw;
  ...
  for(i=0;i<nfiles;i++){
    ...
    tot_area+=area;
    sprintf(filename,"/raid2/ymao/other/CA_drought_0.0625/run_vic/model_output/fluxes_1920_20140930_old2014_adjustT_pivot2014_sens/fluxes_%.5f_%.5f",lat,lon);
    ...
    for(j=0;j<ndays;j++){
      ...
      fscanf(fpin,"%lf %lf %lf %lf %lf %lf",&airT,&sm1,&sm2,&sm3,&swe,&sw);
	if(month>=4 && month<=10)  // if Apr-Oct, add it to this water year
	{
  ...
  for(y=syear;y<=year-1;y++){    // only print water years 1920-2013
    fprintf(fpout,"%d %.4f\n",y,prec_year[y-syear]);
  }
  fclose(fpout);
  printf("%.1f\n",tot_area);
