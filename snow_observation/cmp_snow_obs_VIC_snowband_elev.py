...
elev_bin_interv = 200 # interval of elevation bins, m
...
# data_obs:  # [[site name, abbr, elev(ft), lat, lon], 
             # np.array(year, month, SWE(inch)),
             # [lat_grid, lon_grid],
             # np.array from vic output (year, month, day, SWE for the snowband with the closest elev(m)]]
             # the closest snowband elev(m)
...
for i in range(nobs_orig):
	...
	if os.path.isfile(filepath)==False: # if grid cell not in the study domain, append -1
		...
		if cellnum_list[cell_ind,1]!=latlon_list[cell_ind,0] or cellnum_list[cell_ind,2]!=latlon_list[cell_ind,1] or cellnum_list[cell_ind,0]!=snowband[cell_ind,0]:  # make sure we are reading the correct snowband grid cell
			...
			data_obs[i].append(elev[band_ind]) # closest snowband elevation (m)
...
data_avg = [] # [[site name, abbr, elev(ft), lat, lon], 
              ...
              # elevation of closest snowband (m)
...
for i in range(nobs):
	...
	# closest snowband elev (m)
	data_avg[i].append(data_obs[i][4])
...
###################################################################
####### making binned plot (binned by every 'elev_bin_interv' elevation) #########
###################################################################
# put SWE data into T bins
elev_snowband = np.asarray([row[4] for row in data_avg]) # m
swe_obs_avg = np.asarray([row[1] for row in data_avg]) # mm
swe_vic_avg = np.asarray([row[3] for row in data_avg]) # mm
elev_snowband_bin = (np.around(elev_snowband/float(elev_bin_interv))*elev_bin_interv).astype(int) # rounded snowband elev
elev_snowband_bin_max = np.max(elev_snowband_bin)
elev_snowband_bin_min = np.min(elev_snowband_bin)
nbin = (elev_snowband_bin_max - elev_snowband_bin_min) / elev_bin_interv + 1
swe_obs_binned = []
swe_vic_binned = []
# binned data structure: [[swe1, swe2, swe3, ...] -> elev_min
#                         [swe1, swe2, swe3, ...] -> elev_Tmin+1]
#                         ...
#                         [swe1, swe2, ...] > elev_max ]
for i in range(nbin):
...
for i in range(nobs):
	elevind = (elev_snowband_bin[i] - elev_snowband_bin_min) / elev_bin_interv  # index starts from 0
	swe_obs_binned[elevind].append(swe_obs_avg[i])
	swe_vic_binned[elevind].append(swe_vic_avg[i])
...
for i in range(nbin):
	# vic 
	if bin_swe_stat_vic[i]['n']>=1:
		plt.plot([bin_swe_stat_vic[i]['min'], bin_swe_stat_vic[i]['max']], [elev_snowband_bin_min+i*elev_bin_interv,elev_snowband_bin_min+i*elev_bin_interv], 'r--')
		if bin_swe_stat_vic[i]['n']>=10:
			plt.plot([bin_swe_stat_vic[i]['10th'], bin_swe_stat_vic[i]['90th']], [elev_snowband_bin_min+i*elev_bin_interv,elev_snowband_bin_min+i*elev_bin_interv], 'r-')
			plt.plot([bin_swe_stat_vic[i]['10th'], bin_swe_stat_vic[i]['90th']], [elev_snowband_bin_min+i*elev_bin_interv,elev_snowband_bin_min+i*elev_bin_interv], 'r+')
		plt.plot(bin_swe_stat_vic[i]['median'], elev_snowband_bin_min+i*elev_bin_interv, 'ro')
	if i>=1 and bin_swe_stat_vic[i]['n']>=1 and bin_swe_stat_vic[i-1]['n']>=1: # connect median values
		plt.plot([bin_swe_stat_vic[i-1]['median'], bin_swe_stat_vic[i]['median']], [elev_snowband_bin_min+(i-1)*elev_bin_interv,elev_snowband_bin_min+i*elev_bin_interv], 'r-')
	# obs
	if bin_swe_stat_obs[i]['n']>=1:
		plt.plot([bin_swe_stat_obs[i]['min'], bin_swe_stat_obs[i]['max']], [elev_snowband_bin_min+(i-0.2)*elev_bin_interv,elev_snowband_bin_min+(i-0.2)*elev_bin_interv], 'b--')
		if bin_swe_stat_obs[i]['n']>=10:
			plt.plot([bin_swe_stat_obs[i]['10th'], bin_swe_stat_obs[i]['90th']], [elev_snowband_bin_min+(i-0.2)*elev_bin_interv,elev_snowband_bin_min+(i-0.2)*elev_bin_interv], 'b-')
			plt.plot([bin_swe_stat_obs[i]['10th'], bin_swe_stat_obs[i]['90th']], [elev_snowband_bin_min+(i-0.2)*elev_bin_interv,elev_snowband_bin_min+(i-0.2)*elev_bin_interv], 'b+')
		plt.plot(bin_swe_stat_obs[i]['median'], elev_snowband_bin_min+(i-0.2)*elev_bin_interv, 'bo')
	if i>=1 and bin_swe_stat_obs[i]['n']>=1 and bin_swe_stat_obs[i-1]['n']>=1: # connect median values
		plt.plot([bin_swe_stat_obs[i-1]['median'], bin_swe_stat_obs[i]['median']], [elev_snowband_bin_min+(i-0.2-1)*elev_bin_interv,elev_snowband_bin_min+(i-0.2)*elev_bin_interv], 'b-')

plt.plot(bin_swe_stat_vic[0]['median'], elev_snowband_bin_min, 'ro', label='VIC simulated')
plt.plot(bin_swe_stat_obs[0]['median'], elev_snowband_bin_min-0.2*elev_bin_interv, 'bo', label='Observed')

plt.legend(loc='upper right', fontsize=16)
plt.ylim(elev_snowband_bin_min-elev_bin_interv, elev_snowband_bin_max+elev_bin_interv)
plt.xlabel('Apr 1 SWE (mm)', fontsize=16)
plt.ylabel('Elevation (m)', fontsize=16)
fig.savefig('%s/swe_obs_binned_snowband_elev.png' %output_dir, format='png')
